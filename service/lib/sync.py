...
class Sync(object):
    ...
    def get_data_paged(self, method, subquery=""):
        ...
        while True:
            batch = self.get_data_batch(method, subquery, page, per_page)
            if method not in batch:
                app.logger.warn('{method} not in this batch. Current page {page}. Skipping {method}.'
                                .format(method=method, page=page))
                break
            if len(batch) > 0:
                data.extend(batch)
                page += 1
